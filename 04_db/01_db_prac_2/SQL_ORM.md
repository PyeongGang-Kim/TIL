...
2. user 레코드 생성
   ...
   ```python
   # orm
   user = User(first_name='평강', last_name='김', age=28, country='경상남도', phone='010-6371-3888', balance=30)
   user.save()
   ```

   ```sql
   -- sql
   INSERT INTO users_user (first_name, last_name, age, country, phone, balance) VALUES('평강', '김', 28, '경상남도', '010-6371-3888', 50);
   ```
   ...
   * 하나의 레코드를 빼고 작성 후 `NOT NULL` constraint 오류를 orm과 sql에서 모두 확인 해보세요.
     ...
     ```sql
     -- sql
     INSERT INTO users_user VALUES('평강', '김', 28, '경상남도', '010-6371-3888', 20);
     table users_user has 7 columns but 6 values were supplied
     ```
...
6. 나이가 30이거나 성이 김씨인 사람?
   ...
   갯수는 len(User.objects.filter(age=30)|User.objects.filter(last_name='김'))
   ```
